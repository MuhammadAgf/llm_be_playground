...
async def weather_tool(state: QueryState) -> QueryState:
    ...
    try:
        location_query = ','.join([x for x in [
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(geocode_url, params=geocode_params) as geocode_response:
        ...
        state.result = f"Error fetching weather data (status: {e.status} check your API key or https://openweathermap.org is down)"
    except Exception as e:
        state.result = f"Error processing weather request (status: {e.status} check your API key or https://openweathermap.org is down)"
