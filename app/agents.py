...
from app.models import Intent, LocationInformation, MATH, ASK_ABOUT_WEATHER, GENERIC_QUESTION
...
intent_agent = Agent(
    ...
    system_prompt=(
        ...
        f"Choose from these options: `{MATH}`, `{ASK_ABOUT_WEATHER}`, and `{GENERIC_QUESTION}`."
    ),
    output_type=Intent,
)
...
def validate_result(ctx: RunContext[None], result: Intent) -> Intent:
    if result.intent not in [MATH, ASK_ABOUT_WEATHER, GENERIC_QUESTION]:
        raise ModelRetry(
            f"Invalid action. Please choose from `{MATH}`, `{ASK_ABOUT_WEATHER}`, `{GENERIC_QUESTION}`"
        )
    return result
